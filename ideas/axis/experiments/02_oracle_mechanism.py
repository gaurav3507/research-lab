"""GATE 2 (positive control) --- a pure MECHANISM shift must load off-diagonal.

Simulate an environment pair whose only real change is off-diagonal (how channels
drive each other), fit VAR(1) in each, and check that AXIS flags the change as
off-diagonal and NOT diagonal. This is the positive control: if a real mechanism
change does not show up off-diagonal, the test has no power and cannot predict
when methods fail.

Run on the A100 (or anywhere with numpy/scipy). Imports only from ideas/axis/.

    python experiments/02_oracle_mechanism.py
"""
...
def run_one(seed, magnitude, cfg):
    ...
    # GATE 2: the off-diagonal change must be detected, diagonal must stay clean.
    passed = (summary["offdiag_reject"] > 0) and (summary["diag_reject"] <= FP_BUDGET)
...
def main():
    ...
    records = []
    print(f"GATE 2 (mechanism -> off-diagonal), threshold={cfg['threshold']}, "
    ...
    out_path = os.path.join(results_dir, "02_oracle_mechanism.json")
